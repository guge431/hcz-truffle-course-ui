    ...
    sepolia: {
      provider: () =>
        new HDWalletProvider(
          process.env.MNEMONIC, // 你的助记词
          `https://sepolia.infura.io/v3/${process.env.INFURA_PROJECT_ID}` // Infura 的 Sepolia 端点
        ),
      network_id: 11155111, // Sepolia 的 network id
      gas: 6000000, //交易最多可使用的 gas，防止交易失败
      confirmations: 2, //等待多少区块确认后才继续部署流程（提高部署稳定性）
      timeoutBlocks: 200, //等待多少区块还没被确认就认为部署失败
      skipDryRun: true, //是否跳过部署前的“模拟部署”步骤（测试网建议设为 true）
